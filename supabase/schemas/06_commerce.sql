...
create table supply_contract_request (
  ...
  --
  -- Denormalised on purpose. app_user carries one policy, self_read, so a
  -- pengurus cannot read the buyer's row and a join would come back null.
  -- Copying the two fields the cooperative needs onto the request keeps that
  -- boundary intact: browsing the catalogue stays anonymous, and identity is
  -- disclosed only by the act of asking a named cooperative for produce.
